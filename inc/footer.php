<footer class="footer">

      <div class="container footer-container">

        <div class="row py-md-5 py-5">
          <div class="col-12 col-md-10 mx-md-auto col-lg-5 px-lg-1 pl-5 px-md-0  ">
            <div>
              <label class="footer-text-size text-pink mb-0">
                ขายง่ายง่าย
              </label>
              <label class="footer-text-size-2 text-blue mb-0">
                ดอทคอม
              </label>
            </div>
            <div class="pr-lg-5 pr-md-5">
              <label class="mb-lg-0 mt-lg-2">
                123/32 ถนนประชาสงเคราะห์ เขตห้วยขวาง แขวงสามเสนนอก กรุงเทพมหานคร 10300 โทรศัพท์ 02 123 4567 แฟกซ์ 02 123 4566 อีเมล contact@kaiyngayngay.com
              </label>
            </div>
          </div>
          <div class="col-12 col-md-10 mx-md-auto col-lg-3 mt-md-2 ml-lg-4 mt-2 pl-md-0 pl-5">
              <div>
                <h5>ช่วยเหลือ</h5>
              </div>
              <div class="ml-lg-3 mt-lg-3 ml-md-3 mt-md-3 mt-3 ml-3" style="line-height:15px;">
                <a class="text-black hover-pink" href="#"><label class="mb-3">ติดต่อเรา</label></a><br>
                <a class="text-black hover-pink" href="#"><label class="mb-3">คำถามที่พบบ่อย</label></a><br>
                <a class="text-black hover-pink" href="#"><label class="mb-3">นโยบายการใช้งาน</label></a><br>
                <a class="text-black hover-pink" href="complain.php"><label class="mb-3">ข้อเสนอแนะ</label></a><br>
              </div>
          </div>
          <div class="col-12 col-md-10 mx-md-auto col-lg-3 mt-md-2 ml-lg-4 mt-2 pl-md-0  pl-5">
              <div>
                <h5>เกี่ยวกับเรา</h5>
              </div>
              <div class="ml-lg-3 mt-lg-3 ml-md-3 mt-md-3 mt-3 ml-3" style="line-height:15px;">
                <a class="text-black hover-pink" href="blank_page.php"><label class="mb-3">เกี่ยวกับ ขายง่ายง่ายดอทคอม</label></a><br>
                <a class="text-black hover-pink" href="contact.php"><label class="mb-3">ติดต่อเรา</label></a><br>
                <a class="text-black hover-pink" href="blank_page.php"><label class="mb-3">ลงประกาศโฆษณา</label></a><br>
              </div>
          </div>
        </div>

      </div>

      <div class="second-container pt-3 pb-md-2 pl-md-5 px-5">
        <div class="container footer-container">
          <label class="text-white">
            All Right Reserved 2018 Copyright Kaiyngayngay.com
          </label>
        </div>
      </div>

</footer>
