<!doctype html>
<html lang="en">
  <head>
    <?php include_once('inc/header.php'); ?>
  </head>

  <body style="background-color:#EEE;">

    <?php include_once('inc/navbar.php'); ?>

<div class="container nav-container">

      <div class="row row-content mt-4 mb-5 mx-0">


        <div class="col-lg-4 col-10 px-0 mx-lg-auto mx-auto ">

          <div class="mb-3 ml-lg-0 ml-md-0 ml-3">
            <a class="text-black" href="index.php">หน้าหลัก</a> > <label class="text-blue"> แก้ไขข้อมูลส่วนตัว</label>
          </div>


          <div class="mb-3 ml-lg-0 ml-md-0 ml-3 editprofile-btn">
            <a class="mx-auto mx-md-auto mx-lg-0 nav-link btn active align-center" href="editprofile.php">
                <p class=" mb-0">
                  <i class="fa fa-pencil fa-lg p-relative" aria-hidden="true">
                  </i>
                  แก้ไขข้อมูลส่วนตัว
                </p>
            </a>
          </div>

          <div class="mb-3 ml-lg-0 ml-md-0 ml-3 editprofile-btn">
            <a class="mx-auto mx-md-auto mx-lg-0 nav-link btn not-active align-center" href="mypost.php">
                <p class=" mb-0">
                  <i class="fa fa-copy fa-lg p-relative" aria-hidden="true">
                  </i>
                  ประกาศของฉัน
                </p>
            </a>
          </div>
          
        </div>


        <div class="mx-md-auto col-lg-8 col-md-11 col-12 px-0">


              <div class="page-title mt-5 ml-lg-0 ml-md-0 ml-3 mb-3 align-sm-center align-md-center">
                <h3 class="text-pink mb-0">
                  แก้ไขข้อมูลส่วนตัว
                </h3>
              </div>

              <div class="mb-lg-2 ml-lg-0 ml-md-0  px-md-0 px-3 align-sm-center align-md-center">
                <label>สามารถแก้ไขข้อมูลส่วนตัวได้ที่แบบฟอร์มด้านล่าง</label>
              </div>

              <div class="mb-lg-5 ml-lg-0 ml-md-0 px-md-0 px-3 align-sm-center align-md-center">
                <h4 class="text-dark">ข้อมูลส่วนตัว</h4>
              </div>



              <div class="mb-5 col-lg-8 col-md-10 col-12 px-0 mx-lg-0 mx-auto">

                <div class="col-12">
                  <div class="form-group row mx-0">
                    <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                      Username :
                    </label>
                    <div class="col-lg-5 col-md-5 col-8 px-0 ml-md-2 mx-auto text-md-left text-center">
                      <label >demo</label>
                    </div>
                    <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">
                    </div>
                  </div>
                </div>


                <div class="col-12">
                  <div class="form-group row mx-0">
                    <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                      รหัสผ่าน :
                    </label>
                    <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                      <input type="password" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                    </div>
                    <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">

                    </div>
                  </div>
                </div>


                <div class="col-12">
                  <div class="form-group row mx-0">
                    <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                      ชื่อ :
                    </label>
                    <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                      <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                    </div>
                    <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">

                    </div>
                  </div>
                </div>


                <div class="col-12">
                  <div class="form-group row mx-0">
                    <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                      นามสกุล :
                    </label>
                    <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                      <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                    </div>
                    <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">
                    </div>
                  </div>
                </div>


                <div class="col-12">
                  <div class="form-group row mx-0">
                    <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                      ชื่อในวงการ :
                    </label>
                    <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                      <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                    </div>
                    <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">

                    </div>
                  </div>
                </div>


                  <div class="col-12">
                      <div class="form-group row mx-0">
                        <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                          เบอร์โทรศัพท์ :
                        </label>
                        <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                          <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                        </div>
                        <div class="mt-2 mt-md-0  ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center plus-btn">
                          <button class="btn plus-btn-style" type="button" style="border-radius:50%;">
                            <i class="fa fa-plus"></i>
                          </button>
                        </div>
                      </div>
                  </div>


                  <div class="col-12">
                    <div class="form-group row mx-0">
                      <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                        อีเมล :
                      </label>
                      <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                        <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                      </div>
                      <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">
                      </div>
                    </div>
                 </div>

          </div>

             <hr>

             <div class="mt-lg-5 mb-lg-5 ml-lg-0 ml-md-0  px-md-0 px-3 align-sm-center align-md-center">
               <h4 class="text-dark">ที่อยู่</h4>
             </div>

             <div class="col-lg-8 col-md-10 col-12 px-0 mx-lg-0 mx-auto">

               <div class="col-12">
                 <div class="form-group row mx-0">
                   <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                     บ้านเลขที่ :
                   </label>
                   <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                     <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                   </div>
                   <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">

                   </div>
                 </div>
               </div>

               <div class="col-12">
                 <div class="form-group row mx-0">
                   <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                     ตำบล :
                   </label>
                   <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                     <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                   </div>
                   <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">

                   </div>
                 </div>
               </div>

               <div class="col-12">
                 <div class="form-group row mx-0">
                   <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                     อำเภอ :
                   </label>
                   <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                     <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                   </div>
                   <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">
                   </div>
                 </div>
               </div>

               <div class="col-12">
                 <div class="form-group row mx-0">
                   <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                     จังหวัด :
                   </label>
                   <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                     <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                   </div>
                   <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">
                   </div>
                 </div>
               </div>


               <div class="col-12">
                 <div class="form-group row mx-0">
                   <label class="mr-md-1 mx-auto col-lg-3 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                     รหัสไปรษณีย์ :
                   </label>
                   <div class="ml-md-2 mx-auto col-lg-5 col-md-5 col-8 px-0">
                     <input type="text" class="form-control form-control-sm overide-w-1024" id="" placeholder="">
                   </div>
                   <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">
                   </div>
                 </div>
               </div>

               <!-- <div class="col-12">
                 <div class="form-group row mx-0">
                   <label class="d-none d-md-none d-lg-block mr-md-0 mx-auto col-lg-4 col-md-3 col-11 align-right align-md-right align-sm-center px-0">
                   </label>
                   <div class="ml-lg-3 ml-md-2 col-lg-5 col-md-6 col-8 mx-auto mx-md-auto px-0">
                     <button class="mx-auto mx-md-auto mx-lg-0 nav-link login-btn align-center">
                         <p class=" mb-0">
                           สมัครสมาชิก
                         </p>
                     </button>
                   </div>
                   <div class="mt-2 ml-md-5 mx-auto col-lg-1 col-md-5 col-12 px-0 align-center">
                   </div>
                 </div>
               </div> -->

               <div class="col-12">
                 <div class="form-group row mx-0">
                   <label class="mr-md-1 mx-auto col-lg-3 col-md-2 col-11 align-right align-md-right align-sm-center px-0">

                   </label>
                   <div class="ml-md-2 mx-auto col-lg-5 col-md-3 col-8 px-0">
                     <button class="mx-auto mx-md-auto mx-lg-0 nav-link login-btn align-center">
                         <p class=" mb-0">
                           สมัครสมาชิก
                         </p>
                     </button>
                   </div>
                   <div class="mt-2 mt-md-0 ml-md-0 mx-auto col-lg-1 col-md-1 col-12 px-0 align-center">
                   </div>
                 </div>
               </div>

            </div>

      </div>


    </div>

</div>


    <?php include_once('inc/footer.php'); ?>
    <?php include_once('inc/footer-script.php'); ?>
    </script>


  </body>
</html>
